...
resource "azurerm_management_lock" "this" {
  ...
  name       = coalesce(var.lock.name, "lock-st-${var.tags.product}-${var.tags.environment}-${local.user_preferred_index}")
...
resource "azurerm_role_assignment" "storage_account" {
  ...
  scope                                  = each.value.scope != null ? each.value.scope : azurerm_storage_account.this.id
...
resource "azurerm_storage_account" "this" {
  ...
  name                              = var.name != null ? var.name : "st${var.tags.product}${var.tags.environment}${local.user_preferred_index}"
