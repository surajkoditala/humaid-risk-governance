...
| Data Ingestion Layer | N/A (infrastructure) | Deterministic | The only component allowed to call the Mock Systems service — see §6 |
...
**Key invariants visible in both diagrams:** the flow originates with the Product Owner, not a data feed (Epic 1); the AI Engine never talks directly to the Risk Committee; no arrow skips the FCRM Analyst review step; Policy Research and Scoring are deterministic, not AI, so only three of the ten epics actually call a language model; and Platform Configuration governs the scoring engine through a human-initiated, audited action, not an automatic connection. That's the "system prepares, humans decide" rule made structural rather than just stated.
